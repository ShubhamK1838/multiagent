  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <div>
          <label className="label block mb-1.5">Parameters Schema (JSON Schema)</label>
          <textarea
            className="input font-mono resize-none text-xs"
            rows={6}
            value={JSON.stringify(tool.parametersSchema ?? {}, null, 2)}
            onChange={e => {
              try { update({ parametersSchema: JSON.parse(e.target.value) }) } catch { /* ignore until valid */ }
            }}
          />
        </div>

        <div>
          <label className="label block mb-1.5">Handler Config (JSON)</label>
          <textarea
            className="input font-mono resize-none text-xs"
            rows={4}
            value={JSON.stringify(tool.handlerConfig ?? {}, null, 2)}
            onChange={e => {
              try { update({ handlerConfig: JSON.parse(e.target.value) }) } catch { /* ignore until valid */ }
            }}
          />
